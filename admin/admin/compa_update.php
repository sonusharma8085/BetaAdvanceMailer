...
<?php 
  ...
  $query = mysqli_query($conn,"SELECT * FROM campagain");
  $sql   = mysqli_query($conn,"SELECT * FROM campagain WHERE id ='".$_GET['id']."'");
  $row   = mysqli_fetch_array($sql);
    ...
    $sql = "update campagain set id = $id, tittle= '$tittle', sub_tittle='$sub_tittle',massage='$massage', image='$destfile' WHERE id =$id";
   ...
   if ($mysql==true) {
    ...
    echo '<script type="text/javascript">alert("Succesfully Data update")
        ...
        echo "<script>location.href='compagainsetup.php'</script>";
    }else{

    echo '<script type="text/javascript">alert("Data not update")
        </script>';
        //header('location:emailserver_update.php');
        echo "<script>location.href='comp_update.php'</script>";
   }
